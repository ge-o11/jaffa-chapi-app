  ...
  return (
    <section id="store" className="py-24" style={{ background: 'var(--navy)' }}>
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center justify-center gap-3 mb-12 p-4 rounded-2xl max-w-xl mx-auto" style={{ background: 'rgba(200,169,110,0.08)', border: '1px solid rgba(200,169,110,0.2)' }}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {products.map(p => (
            <div key={p.name} className="card text-right hover:transform hover:-translate-y-2 transition-all duration-300 cursor-pointer">
              ...
              <div className="text-xs px-2 py-0.5 rounded-full inline-block mb-2" style={{ background: 'rgba(200,169,110,0.15)', color: 'var(--gold)' }}>
                {p.category}
              </div>
              ...
              <div className="mt-4 pt-4 border-t text-xs text-center opacity-50" style={{ borderColor: 'rgba(200,169,110,0.2)', color: 'var(--parchment)' }}>
                📍 רכישה ישירה במקום
              </div>
            </div>
          ))}
